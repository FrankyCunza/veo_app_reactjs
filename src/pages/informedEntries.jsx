...
const InformedEntries = () => {
    let history = useHistory()
    const location = useLocation();
    const [data, setData] = useState(location.state.data)

    const gotoNotice = (item) => {
        history.push("/informedArticle", {data: item})
    };
        ...
        <div className="max-w-3xl mx-auto">
            ...
            <div className="grid grid-cols-2 md:grid-cols-1 gap-4 mt-6">
                {data.articles.length>0 && data.articles.map((el, i) => {
                    return (
                        <div className="w-full bg-white rounded-xl p-6 shadow hover:shadow-xl mt-4 flex cursor-pointer relative" key={'entries'+i} onClick={() => gotoNotice(el)}>
...
export default InformedEntries