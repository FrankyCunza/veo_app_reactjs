...
const AlertWarning = ( { props } ) => {
    ...
    return (
        <div className="fixed w-full h-full top-0 left-0 z-10 flex items-center justify-center">
            <div className="absolute bg-black bg-opacity-20 w-full h-full top-0 left-0"></div>
            <div className="max-w-sm w-full bg-white rounded-xl shadow-2xl px-4 py-14 z-10 text-center relative">
                ...
                <h2 className="font-extrabold text-4xl text-green-600">{props.title}</h2>
                <p className="text-gray-700 mt-4 px-8">{props.message}</p>
                <button type="button" className="bg-blue-500 px-8 mt-4 py-3 font-medium tracking-widest rounded-full text-white" onClick={() => gotoStep(props.route)}>OKAY</button>
            </div>
        </div>
    )
}
